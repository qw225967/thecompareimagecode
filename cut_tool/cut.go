...
func Filter2EvaluateInfo(mInfo MediaInfo) (*Filter2EvlInfo, error) {
	...
	info := Filter2EvlInfo{}
	// 1. interval time
	info.Interval = 30 // keep in 1m ~ 30s
	if info.Interval > 60 || info.Interval < 30 {
	...
	} else if info.ScoreCount == 4 { // 当时长大于1800时认定为4段
		drop := uint64(duration) / 4
...
func catFile(time uint64, prename, name string) {
	...
	cmdArguments := []string{"-y", "-i", prename, "-ss", timeStr, "-t", "30", name}
...
func toYUV(prename, name, WandH string) {
	cmdArguments := []string{"-i", prename, "-ss", "00:00:00", "-t", "30", "-s", WandH, "-pix_fmt", "yuv420p", name}
...
func main() {
	...
	for _, v := range info.CutTimeArr {
		...
		name := strs[0] + "-" + fmt.Sprintf("%02d", i) + ".mkv"
		fmt.Println(name)
		catFile(v.StartTime, path, name)
		//fmt.Println(WandH)
		nameYUV := strs[0] + "-" + fmt.Sprintf("%02d", i) + ".yuv"
		flag := Exists(name)
		if flag {
